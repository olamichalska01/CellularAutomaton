...
void make_new(generation* next, generation* previous)
{
	next -> r = previous -> r;
	next -> c = previous -> c;
	next -> nr = previous -> nr;
...
generation* fast(generation* previous, int count, neighbour how, char to_save, int how_many_to_save, int const_count, int saved)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		make_new(next, previous);
		new_gen(previous, how); 
...
generation* sbs(generation* previous, int count, neighbour how, char to_save, int how_many_to_save)
{
	...
	{	
		...
		{
			new_gen(previous, how);
			fast(previous, count - i - 1, how, to_save, how_many_to_save, count, how_many_to_save - any_left);
			break;
		}
		else
		{
